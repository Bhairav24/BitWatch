...
const SplashScreen = () => {
  ...
  const fetchDataFromApi = async () => {
    try {
      const result = await axios.get(
        "http://ec2-43-204-233-148.ap-south-1.compute.amazonaws.com:5000/admin/getAllSliderImages"
      );
  ...
  if (error) {
    return <div><Skeleton err= {error.message}/> </div>;
  }
        ...
        <main>
          <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl mx-auto">
            <div className="sm:flex sm:justify-between sm:items-center mb-8">
              <BreadCrumb parentPage="Contents" currentPage="Start Screen" />
            </div>
            <div className="flex justify-between items-center mb-7">
           
  <h2 className="text-slate-800 text-3xl font-bold bg-gradient-to-r from-customPink to-customBlue text-transparent bg-clip-text">
    All Splash Screens
  </h2>
  <Button
    modelOpen={() => setIsModalOpen(true)}
    buttonName="Add Splash Screen"
  />
            ...
            <div className="col-span-full xl:col-span-8 bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
              <div className="p-3">
                <div className="grid grid-cols-1 md:grid-cols-3">
                  {data.map((item, index) => (
                    <SplashScreenCard key={index} data={item} />
                  ))}
