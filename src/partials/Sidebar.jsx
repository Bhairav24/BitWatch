...
function Sidebar({ sidebarOpen, setSidebarOpen }) {
  ...
  return (
    <>
      {/* Sidebar backdrop (mobile only) */}
      <div className={`fixed inset-0 bg-slate-900 bg-opacity-100 z-40 lg:hidden transition-opacity duration-200 ${sidebarOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} aria-hidden="true"></div>
      ...
      <div id="sidebar" ref={sidebar} className={`flex flex-col absolute z-40 left-0 top-0 lg:static lg:left-auto lg:top-auto lg:translate-x-0 h-screen overflow-y-scroll lg:overflow-y-auto no-scrollbar w-64 lg:w-20 lg:sidebar-expanded:!w-64 2xl:!w-64 bg-slate-00 p-4 transition-all duration-200 ease-in-out ${sidebarOpen ? 'translate-x-0' : '-translate-x-64'}`}>
        {/* Sidebar header */}
             {/* Close button */}
        <button ref={trigger} className="lg:hidden text-white hover:text-slate-400 pt-10" onClick={() => setSidebarOpen(!sidebarOpen)} aria-controls="sidebar" aria-expanded={sidebarOpen}>
            <span className="sr-only">Close sidebar</span>
            <svg className="w-6 h-6 fill-current" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <path d="M10.7 18.7l1.4-1.4L7.8 13H20v-2H7.8l4.3-4.3-1.4-1.4L4 12z" />
            </svg>
        ...
        <div className="flex justify-center mb-10 pr-3 sm:px-2">
          ...
          <NavLink end to="/dashboard" className="flex flex-col items-center">
            <img src={BitWatchLogo} alt="site-logo" width="70" className='mt-4'/>
                  ...
                  <>
                    ...
                    <div className="lg:hidden lg:sidebar-expanded:block 2xl:block">
                      <ul className={`pl-9 mt-1 ${!open && 'hidden'}`}>
                        ...
                        <li className="mb-1 last:mb-0">
                          <NavLink end to="/splashscreen" className={({ isActive }) => 'block transition duration-150 truncate ' + (isActive ? 'text-zinc-600' : 'text-white hover:text-customPink')}>
                            <span className="text-sm font-medium lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">Splash Screen</span>
                          </NavLink>
                        </li>
                        {/* Other links */}
                      </ul>
